...
fn bench_runtime<R: WasmRuntime>(name: &str, rt: &mut R, tool_source: &str, args_json: &str) {
    println!("\n=== {} ===", name);
...
fn main() {
    ...
    // QuickJS
    let qjs_wasm =
        std::fs::read("bench_tools/quickjs_wasm/quickjs.wasm").expect("quickjs.wasm not found");
    ...
    let php_source =
        "<?php function bench_tool($args) { return ['size' => 64, 'payload' => 'hello']; }";
    ...
    let perl_source =
        "sub bench_tool { my ($args) = @_; return { size => 64, payload => 'hello' }; }";
    bench_runtime(
        "Perl/WASM",
        &mut perl_rt,
        perl_source,
    ...
    // C#
    let csharp_wasm =
        std::fs::read("bench_tools/csharp_wasm/dotnet.wasm").expect("dotnet.wasm not found");
    let mut csharp_rt = CSharpRuntime::new(&csharp_wasm).expect("CSharpRuntime::new");
    csharp_rt.init().expect("init failed");
    let csharp_source =
        "static object bench_tool(dynamic args) { return new { size = 64, payload = \"hello\" }; }";
    bench_runtime(
        "C#/WASM",
        &mut csharp_rt,
        csharp_source,
    ...
    // Julia
    let julia_wasm =
        std::fs::read("bench_tools/julia_wasm/julia.wasm").expect("julia.wasm not found");
    let mut julia_rt = JuliaRuntime::new(&julia_wasm).expect("JuliaRuntime::new");
    julia_rt.init().expect("init failed");
    let julia_source =
        "function bench_tool(args) Dict(\"size\" => 64, \"payload\" => \"hello\") end";
    bench_runtime(
        "Julia/WASM",
        &mut julia_rt,
        julia_source,
    ...
    let rust_wasm = std::fs::read("bench_tools/rust_wasm/example_tool.wasm")
        .expect("example_tool.wasm not found");
    ...
    bench_runtime(
        "Rust/WASM",
        &mut rust_rt,
        "bench_tools/rust_wasm/example_tool.wasm",
        r#"{"text": "hello"}"#,
    );
