...
"#;
    ...
    rt.register_tool("compute_tool", source).expect("register failed");

    // Verify correctness
    let result = rt.call_tool("compute_tool", r#"{"n": 100}"#).expect("call failed");
    println!("Result for n=100: {}", result);
    assert!(result.contains("328350"), "Expected sum=328350 for n=100, got: {}", result);

    let result2 = rt.call_tool("compute_tool", r#"{"n": 1000}"#).expect("call 2 failed");
    println!("Result for n=1000: {}", result2);
    assert!(result2.contains("332833500"), "Expected sum=332833500 for n=1000, got: {}", result2);

    // Benchmark with actual work
    let iters = 100;
    let start = Instant::now();
    for _ in 0..iters {
        let _ = rt.call_tool("compute_tool", r#"{"n": 100}"#).expect("call failed");
    }
    ...
    rt.register_tool("trivial_tool", trivial_source).expect("register trivial failed");
    
    let start2 = Instant::now();
    for _ in 0..iters {
        let _ = rt.call_tool("trivial_tool", r#"{}"#).expect("trivial call failed");
    }
    ...
    println!("\nOverhead of computation: {:.2} µs", per_call_us - per_call_trivial_us);
