...
pub fn encrypt_token(token: &OAuth2Token) -> Result<Vec<u8>, String> {
    let key_store = get_encryption_key()
        .lock()
...
pub fn load_token_encrypted(connector_id: &str, path: &str) -> Result<OAuth2Token, String> {
    let encrypted =
        std::fs::read(path).map_err(|e| format!("Failed to read encrypted token: {}", e))?;
...
pub fn send_webhook(
    ...
    let config = get_connector_registry()
        .lock()
        .ok()
        .and_then(|r| r.get(connector_id).cloned())
        .ok_or_else(|| format!("Connector not found: {}", connector_id))?;

    let webhook_config = config.webhook_config.ok_or_else(|| {
        format!(
            "Connector {} does not have webhook configured",
            connector_id
        )
    })?;
    ...
    for attempt in 1..=max_attempts {
        let mut req_builder =
            ureq::post(&webhook_config.endpoint).timeout(std::time::Duration::from_secs(15));
...
pub fn generate_authorize_url(
    ...
    if let Ok(mut state_store) = get_state_store().lock() {
        ...
        if state_store.len() >= MAX_STATE_ENTRIES {
            tracing::warn!(
                "OAuth2 state store full ({} entries), clearing stale entries",
                MAX_STATE_ENTRIES
            );
            state_store.clear();
        }
...
pub fn refresh_token(connector_id: &str) -> Result<OAuth2Token, String> {
    ...
    let refresh_token = token.refresh_token.ok_or("No refresh token available")?;
...
pub fn microsoft_connector_template(
    client_id: &str,
...
pub fn handle_connector_call(params: &Value) -> Result<Value, String> {
    ...
    #[cfg(feature = "oauth2")]
    {
        let connector_id = params["connectorId"]
            .as_str()
...
mod tests {
    ...
    fn test_handle_connector_call_no_feature() {
        ...
        assert!(
            result.is_err(),
            "should error when oauth2 feature is disabled"
        );
        #[cfg(feature = "oauth2")]
        assert!(
            result.is_err(),
            "unregistered connector should return an error"
        );
    }
