...
impl Marketplace {
    ...
    pub fn search(&self, query: &SearchQuery) -> SearchResults {
        let mut results: Vec<PluginMetadata> = self
            ...
            .filter(|plugin| {
                ...
                // Tag filter
                if !query.tags.is_empty() && !query.tags.iter().all(|t| plugin.tags.contains(t)) {
                    return false;
                }
    ...
    pub fn install(&mut self, plugin_id: &str) -> Result<InstalledPlugin, String> {
        ...
        if metadata.id.contains("..") || metadata.id.contains('/') || metadata.id.contains('\\') {
            return Err(format!(
                "Invalid plugin ID '{}': must not contain path separators or '..'",
                metadata.id
            ));
        }
...
mod tests {
    ...
    fn test_marketplace_search() {
        ...
        marketplace.index.plugins.push(make_plugin(
            "test.plugin1",
            "Test Plugin 1",
            "Test Author",
            vec!["test", "example"],
    ...
    fn test_marketplace_search_by_tag() {
        ...
        assert_eq!(
            results.total, 2,
            "Should find 2 plugins with 'database' tag"
        );
        assert!(results
            .plugins
            .iter()
            .all(|p| p.tags.contains(&"database".to_string())));
    }
    ...
    fn test_marketplace_search_sort_by_rating() {
        ...
        assert_eq!(
            results.plugins[0].id, "p2",
            "Highest rating should be first"
        );
    ...
    fn test_marketplace_search_pagination() {
        ...
        assert_eq!(
            results.plugins.len(),
            3,
            "Should return 3 plugins for second page"
        );
    }
    ...
    fn test_marketplace_submit_review() {
        ...
        let result =
            marketplace.submit_review("p1", "Reviewer", 5, "Excellent plugin!".to_string());
        assert!(
            result.is_ok(),
        ...
        assert!(
            (plugin.rating - 5.0).abs() < f32::EPSILON,
            "Rating should be updated to 5.0, got {}",
            plugin.rating
        );
    ...
    fn test_marketplace_stats() {
        ...
        assert_eq!(
            stats.total_downloads, 450,
            "Total downloads should be 100 + 200 + 150"
        );
    }
    ...
    fn test_marketplace_new_with_existing_files() {
        ...
        installed_map.insert(
            "persisted.plugin".to_string(),
            InstalledPlugin {
                metadata: make_plugin(
                    "persisted.plugin",
                    "Persisted",
                    "Author",
                    vec![],
        ...
        std::fs::write(
            ...
            serde_json::to_string(&installed_map).unwrap(),
        )
        .unwrap();
    ...
    fn test_update_index() {
        ...
        assert!(
            ...
            "index.json should be persisted"
        );
    }
    ...
    fn test_search_sort_unknown_field() {
        ...
        marketplace
            .index
            .plugins
            .push(make_plugin("p1", "P1", "A", vec![], 100, 4.0, false));
    ...
    fn test_install_already_installed() {
        ...
        assert!(
            ...
            "Expected 'already installed' error, got: {}",
            err
        );
    }
    ...
    fn test_uninstall_removes_directory() {
        ...
        marketplace
            .index
            .plugins
            .push(make_plugin("rm.plugin", "Rm", "A", vec![], 0, 0.0, false));

        let installed = marketplace.install("rm.plugin").unwrap();
        assert!(
            installed.install_path.exists(),
            "Install directory should exist after install"
        );

        marketplace.uninstall("rm.plugin").unwrap();
        assert!(
            !installed.install_path.exists(),
            "Install directory should be removed after uninstall"
        );
    }
    ...
    fn test_list_installed() {
        ...
        marketplace
            .index
            .plugins
            .push(make_plugin("a", "A", "Auth", vec![], 0, 0.0, false));
        marketplace
            .index
            .plugins
            .push(make_plugin("b", "B", "Auth", vec![], 0, 0.0, false));
    ...
    fn test_set_enabled() {
        ...
        marketplace
            .index
            .plugins
            .push(make_plugin("toggle", "Toggle", "A", vec![], 0, 0.0, false));
    ...
    fn test_set_enabled_not_installed() {
        ...
        assert!(
            ...
            "Expected 'not installed' error, got: {}",
            err
        );
    }
    ...
    fn test_submit_review_invalid_rating() {
        ...
        marketplace
            .index
            .plugins
            .push(make_plugin("rated", "Rated", "A", vec![], 0, 0.0, false));
    ...
    fn test_check_updates_no_update_needed() {
        ...
        marketplace
            .index
            .plugins
            .push(make_plugin("same", "Same", "A", vec![], 0, 0.0, false));
