//! C#/.NET WASM runtime — C# tool execution via .NET runtime compiled to WASM.
//!
//! Uses a WASI reactor build of the .NET IL interpreter running inside Wasmer.
...
impl CSharpRuntime {
    ...
    pub fn exec_and_get_output(&mut self, src: &str) -> Result<String, Box<dyn Error>> {
        ...
        if rc != 0 {
            return Err(format!("C# error: {}", output.trim()).into());
        }
        Ok(output)
    }
}
...
impl WasmRuntime for CSharpRuntime {
    ...
    fn register_tool(&mut self, name: &str, source: &str) -> Result<(), Box<dyn Error>> {
        ...
        if result[0].unwrap_i32() != 0 {
            let output = self.get_output()?;
            return Err(format!("Failed to register C# tool: {}", output.trim()).into());
        }
    ...
    fn call_tool(&mut self, name: &str, args_json: &str) -> Result<String, Box<dyn Error>> {
        if !self.tools.contains_key(name) {
            return Err(format!("Unknown C# tool: {}", name).into());
        }
        ...
        if rc != 0 {
            return Err(format!("C# tool error: {}", output.trim()).into());
        }
    ...
    fn destroy(&mut self) -> Result<(), Box<dyn Error>> {
        let destroy_fn = self.instance.exports.get_function("dotnet_wasi_destroy")?;
...
mod tests {
    ...
    fn wasm_path() -> std::path::PathBuf {
        ...
        p.push("bench_tools/csharp_wasm/dotnet.wasm");
        p
    }

    #[test]
    #[ignore]
    fn test_csharp_basic_exec() {
