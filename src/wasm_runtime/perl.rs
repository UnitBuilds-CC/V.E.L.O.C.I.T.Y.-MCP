...
impl PerlRuntime {
    ...
    fn get_output(&mut self) -> Result<String, Box<dyn Error>> {
        ...
        let get_len_fn = self
            .instance
            .exports
            .get_function("perl_wasi_get_output_len")?;
...
impl WasmRuntime for PerlRuntime {
    fn init(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        self.call_tool_fn = Some(
            self.instance
                .exports
                .get_function("perl_wasi_call_tool")?
                .clone(),
        );
        ...
        self.call_tool_binary_fn = self
            .instance
            .exports
            .get_function("perl_wasi_call_tool_binary")
            .ok()
            .cloned();
        Ok(())
    }
    ...
    fn register_tool(&mut self, name: &str, source: &str) -> Result<(), Box<dyn Error>> {
        ...
        let register_fn = self
            .instance
            .exports
            .get_function("perl_wasi_register_tool")?;
    ...
    fn call_tool_binary(&mut self, name: &str, args_tlv: &[u8]) -> Result<String, Box<dyn Error>> {
        ...
        // Single WASI call: perl_wasi_call_tool_binary(tlv_ptr, tlv_len, name_ptr, name_len)
        let result = call_fn.call(
            &mut self.store,
            &[
                Value::I32(ARGS_SLOT as i32),
...
mod tests {
    ...
    fn test_perl_basic_exec() {
        ...
        let output = rt
            .exec_and_get_output("print 'hello from perl';")
            .expect("exec failed");
    ...
    fn test_perl_tool_roundtrip() {
        ...
        let result = rt
            .call_tool("greet", r#"{"name": "Perl"}"#)
            .expect("call failed");
        assert!(
            result.contains("Hello, Perl!"),
            "unexpected result: {}",
            result
        );
