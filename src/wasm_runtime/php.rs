...
impl PhpRuntime {
    ...
    fn get_output(&mut self) -> Result<String, Box<dyn Error>> {
        ...
        let get_len_fn = self
            .instance
            .exports
            .get_function("php_wasi_get_output_len")?;
...
impl WasmRuntime for PhpRuntime {
    fn init(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        self.call_tool_fn = Some(
            self.instance
                .exports
                .get_function("php_wasi_call_tool")?
                .clone(),
        );
        ...
        self.call_tool_binary_fn = self
            .instance
            .exports
            .get_function("php_wasi_call_tool_binary")
            .ok()
            .cloned();
        Ok(())
    }
    ...
    fn register_tool(&mut self, name: &str, source: &str) -> Result<(), Box<dyn Error>> {
        ...
        let register_fn = self
            .instance
            .exports
            .get_function("php_wasi_register_tool")?;
...
mod tests {
    ...
    fn test_php_basic_exec() {
        ...
        let output = rt
            .exec_and_get_output("<?php echo 'hello from php';")
            .expect("exec failed");
    ...
    fn test_php_tool_roundtrip() {
        ...
        let result = rt
            .call_tool("greet", r#"{"name": "PHP"}"#)
            .expect("call failed");
        assert!(
            result.contains("Hello, PHP!"),
            "unexpected result: {}",
            result
        );
