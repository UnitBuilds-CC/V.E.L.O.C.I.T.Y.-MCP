...
impl QuickJsRuntime {
    ...
    pub fn eval_at(&mut self, code_ptr: i32, code_len: i32) -> Result<i32, Box<dyn Error>> {
        let qjs_eval_fn = self
            .qjs_eval_fn
            .as_ref()
            .ok_or("qjs_eval not cached — call init() first")?;
        let result = qjs_eval_fn.call(
            &mut self.store,
            &[
                Value::I32(code_ptr),
                Value::I32(code_len),
    ...
    pub fn write_to_exec_slot(&mut self, data: &[u8]) -> Result<(), Box<dyn Error>> {
        ...
        self.memory
            .view(&self.store)
            .write(EXEC_SLOT + data.len() as u64, &[0u8])?;
        Ok(())
    }
    ...
    pub fn check_exception(&mut self, handle: i32) -> Result<Option<String>, Box<dyn Error>> {
        let is_exc_fn = self
            .qjs_is_exception_fn
            .as_ref()
            .ok_or("qjs_is_exception not cached")?;
        ...
        let exc_fn = self
            .qjs_get_exception_fn
            .as_ref()
            .ok_or("qjs_get_exception not cached")?;
        ...
        Ok(Some(
            msg.unwrap_or_else(|| "Unknown JS exception".to_string()),
        ))
    }
    ...
    fn read_string_from_handle(&mut self, handle: i32) -> Result<Option<String>, Box<dyn Error>> {
        let get_str_fn = self
            .qjs_get_string_len_fn
            .as_ref()
            .ok_or("qjs_get_string_len not cached")?;

        let len_ptr = self.len_ptr_slot as i32;

        let cstr_result =
            get_str_fn.call(&mut self.store, &[Value::I32(handle), Value::I32(len_ptr)])?;
        ...
        let result = if cstr_ptr != 0 {
            ...
            self.memory
                .view(&self.store)
                .read(len_ptr as u64, &mut len_buf)?;
            let str_len = u32::from_le_bytes(len_buf) as usize;
            let mut str_buf = vec![0u8; str_len];
            self.memory
                .view(&self.store)
                .read(cstr_ptr as u64, &mut str_buf)?;
    ...
    pub fn free_value(&mut self, handle: i32) -> Result<(), Box<dyn Error>> {
        let free_fn = self
            .qjs_free_value_fn
            .as_ref()
            .ok_or("qjs_free_value not cached")?;
    ...
    pub fn bench_cold_start(wasm_bytes: &[u8]) -> f64 {
        ...
        for _ in 0..cold_iters {
            ...
            let qjs_init_fn = instance
                .exports
                .get_function("qjs_init")
                .unwrap()
                .typed::<(), i32>(&store)
                .unwrap();
    ...
    fn call_tool_batched(&mut self, args_json: &str) -> Result<String, Box<dyn Error>> {
        ...
        let wrapper = self
            .wrapper_handle
            .ok_or("wrapper not cached — call register_tool first")?;
        ...
        let tool_fn = self
            .qjs_tool_call_fn
            .as_ref()
            .ok_or("qjs_tool_call not cached")?;
        let ret = tool_fn.call(
            &mut self.store,
            &[
        ...
        self.memory
            .view(&self.store)
            .read(result_ptr as u64, &mut buf)?;
    ...
    fn free_cstring(&mut self, ptr: u32) -> Result<(), Box<dyn Error>> {
        let f = self
            .qjs_free_cstring_fn
            .as_ref()
            .ok_or("qjs_free_cstring not cached")?;
...
impl WasmRuntime for QuickJsRuntime {
    fn init(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        self.qjs_is_exception_fn = self
            .instance
            .exports
            .get_function("qjs_is_exception")
            .ok()
            .cloned();
        self.qjs_get_exception_fn = self
            .instance
            .exports
            .get_function("qjs_get_exception")
            .ok()
            .cloned();
        self.qjs_get_string_len_fn = self
            .instance
            .exports
            .get_function("qjs_get_string_len")
            .ok()
            .cloned();
        self.qjs_free_value_fn = self
            .instance
            .exports
            .get_function("qjs_free_value")
            .ok()
            .cloned();
        self.qjs_new_string_fn = self
            .instance
            .exports
            .get_function("qjs_new_string")
            .ok()
            .cloned();
        ...
        self.qjs_tool_call_fn = self
            .instance
            .exports
            .get_function("qjs_tool_call")
            .ok()
            .cloned();
        self.qjs_tool_call_binary_fn = self
            .instance
            .exports
            .get_function("qjs_tool_call_binary")
            .ok()
            .cloned();
        self.qjs_free_cstring_fn = self
            .instance
            .exports
            .get_function("qjs_free_cstring")
            .ok()
            .cloned();
    ...
    fn register_tool(&mut self, name: &str, source: &str) -> Result<(), Box<dyn Error>> {
        ...
        self.memory
            .view(&self.store)
            .write(PROP_NAME_SLOT, prop_name)?;
        let wrapper_result = get_prop_fn.call(
            &mut self.store,
            &[Value::I32(global_handle), Value::I32(PROP_NAME_SLOT as i32)],
        )?;
    ...
    fn call_tool(&mut self, name: &str, args_json: &str) -> Result<String, Box<dyn Error>> {
        ...
        let new_str_fn = self
            .qjs_new_string_fn
            .as_ref()
            .ok_or("qjs_new_string not cached")?;
        let args_result = new_str_fn.call(
            &mut self.store,
            &[
                Value::I32(EXEC_SLOT as i32),
                Value::I32(args_bytes.len() as i32),
        ...
        let call_fn = self.qjs_call_fn.as_ref().ok_or("qjs_call not cached")?;
...
mod tests {
    ...
    fn test_quickjs_basic_exec() {
        ...
        let output = rt
            .eval_to_string("'hello from quickjs'")
            .expect("exec failed");
    ...
    fn test_quickjs_call_tool_via_api() {
        ...
        let result = rt
            .call_tool("greet", r#"{"name": "QuickJS"}"#)
            .expect("call failed");
    ...
    fn test_quickjs_custom_batched_path() {
        ...
        let result = rt
            .call_tool("greet", r#"{"name": "Batched", "n": 41}"#)
            .expect("call failed");
        assert!(
            result.contains("Hello, Batched!") && result.contains(r#""n":42"#),
            "unexpected: {}",
            result
        );
        ...
        let result3 = rt
            .call_tool("greet", r#"{"name": "again", "n": 1}"#)
            .expect("call after error failed");
    ...
    fn test_destroy_isolation() {
        ...
        let iters: usize = std::env::var("QJS_ISO_ITERS")
            .ok()
