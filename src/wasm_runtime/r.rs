//! R/WASM runtime — R tool execution via R interpreter compiled to WASM.
//!
//! Uses a WASI reactor build of R (based on webR) running inside Wasmer.
...
impl RRuntime {
    ...
    fn exec(&mut self, src: &str) -> Result<i32, Box<dyn Error>> {
        ...
        let exec_fn = self.instance.exports.get_function("r_wasi_exec")?;
    ...
    fn get_output(&mut self) -> Result<String, Box<dyn Error>> {
        let get_output_fn = self.instance.exports.get_function("r_wasi_get_output")?;
        let get_len_fn = self.instance.exports.get_function("r_wasi_get_output_len")?;
    ...
    pub fn exec_and_get_output(&mut self, src: &str) -> Result<String, Box<dyn Error>> {
        ...
        if rc != 0 {
            return Err(format!("R error: {}", output.trim()).into());
        }
        Ok(output)
    }
}

impl WasmRuntime for RRuntime {
    fn init(&mut self) -> Result<(), Box<dyn Error>> {
        let init_fn = self.instance.exports.get_function("r_wasi_init")?
            .typed::<(), i32>(&self.store)?;
        let result = init_fn.call(&mut self.store)?;
        if result != 0 {
            return Err(format!("r_wasi_init() failed with code {}", result).into());
        }
        self.call_tool_fn = Some(self.instance.exports.get_function("r_wasi_call_tool")?.clone());
        // Load binary protocol function if available (optional, for optimized path)
        self.call_tool_binary_fn = self.instance.exports.get_function("r_wasi_call_tool_binary").ok().cloned();
        Ok(())
    }
    ...
    fn register_tool(&mut self, name: &str, source: &str) -> Result<(), Box<dyn Error>> {
        ...
        let register_fn = self.instance.exports.get_function("r_wasi_register_tool")?;
        ...
        if result[0].unwrap_i32() != 0 {
            let output = self.get_output()?;
            return Err(format!("Failed to register R tool: {}", output.trim()).into());
        }
    ...
    fn call_tool(&mut self, name: &str, args_json: &str) -> Result<String, Box<dyn Error>> {
        if !self.tools.contains_key(name) {
            return Err(format!("Unknown R tool: {}", name).into());
        }
        ...
        if rc != 0 {
            return Err(format!("R tool error: {}", output.trim()).into());
        }
    ...
    fn call_tool_binary(&mut self, name: &str, args_tlv: &[u8]) -> Result<String, Box<dyn Error>> {
        ...
        // Single WASI call: r_wasi_call_tool_binary(tlv_ptr, tlv_len, name_ptr, name_len)
        let result = call_fn.call(&mut self.store, &[
            Value::I32(ARGS_SLOT as i32),
...
mod tests {
    ...
    fn wasm_path() -> std::path::PathBuf {
        ...
        p.push("bench_tools/r_wasm/r.wasm");
        p
    }

    #[test]
    #[ignore]
    fn test_r_basic_exec() {
        let wasm = std::fs::read(wasm_path()).expect("R WASM not found");
        let mut rt = RRuntime::cold_start(&wasm).expect("cold start failed");

        let output = rt.exec_and_get_output("cat('hello from R')").expect("exec failed");
        assert_eq!(output.trim(), "hello from R");
    ...
    fn test_r_tool_roundtrip() {
        ...
        assert!(result.contains("Hello, R!"), "unexpected result: {}", result);
