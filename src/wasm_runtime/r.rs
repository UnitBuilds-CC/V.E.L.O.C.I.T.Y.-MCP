...
impl RRuntime {
    ...
    fn get_output(&mut self) -> Result<String, Box<dyn Error>> {
        ...
        let get_len_fn = self
            .instance
            .exports
            .get_function("r_wasi_get_output_len")?;
...
impl WasmRuntime for RRuntime {
    fn init(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        self.call_tool_binary_fn = self
            .instance
            .exports
            .get_function("r_wasi_call_tool_binary")
            .ok()
            .cloned();
        Ok(())
    }
...
mod tests {
    ...
    fn test_r_basic_exec() {
        ...
        let output = rt
            .exec_and_get_output("cat('hello from R')")
            .expect("exec failed");
    ...
    fn test_r_tool_roundtrip() {
        ...
        let result = rt
            .call_tool("greet", r#"{"name": "R"}"#)
            .expect("call failed");
        assert!(
            result.contains("Hello, R!"),
            "unexpected result: {}",
            result
        );
